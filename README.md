...
- 允许动态地进行调用栈的查询，随时查询调用历史，就像真实中人脑回溯自己的想法一般。
---
## 执行器虚拟机模块及各公开接口
...
- 枚举类 **command_type**
  ...
  |gstack|data_type_unsigned_base|在调用栈中查询在此之前的调用信息|  
...
- **基础指令的操作数格式**
  ...
  |gstack|2|往前所要查询调用信息的数量变量, 调用信息保存变量| 
- 所有指令执行函数遵循统一的函数签名
  ```
  void func_name
  (
...
## *我的个人想法*
*这个项目，源自我对一个问题持续而固执的追问：我们如何才能精确地模拟思维本身？*

*我不是想造一个更聪明的聊天机器人，也不是想用统计学去拟合一个看似智能的黑箱。我想要捕捉的，是那个在我们头脑中发生的、清晰的、可以被一步步推演的逻辑过程——打草稿、下判断、做修改、撤销、调整计划。这些最基础的思维操作，能不能被提炼成一套足够精炼、足够根本的指令集，然后用最严谨的代码把它实现出来？*

*于是就有了现在的这个项目。*

*它的每一个核心设计，都对应着一种我心目中思维的基本动作：*

*变量，是我们思考时随手记下的草稿纸；*

*cmp 和跳转，是我们做判断、做选择时的那个“如果……那么……”；*

*mov，是我们把一个想法从一个地方传递到另一个地方；*

*call 和 ret，是我们把一个大问题拆成小问题，分别解决后再合并；*

*acmd、scmd、icmd、dcmd，则是我们突发的灵感、对想法的修正、插入和撤销——也就是思维对自己本身的重塑；*

*gcmd 和 gstack，是我们对自己思考过程的回顾和审视（“我刚才到底是怎么想的？”）。*

*这些指令放在一起，就构成了一个能够思考、能够反思、并且能够在运行时改变自己思考逻辑的程序。它不是一个死板的规则引擎，而是一个思维的可执行模型。*

*在设计这个系统的过程中，我受到了物理学中热力学第二定律的启发：一个封闭的系统，如果没有外部能量输入，只会变得越来越混乱，最终趋向无序。思考也是如此。如果我们只让一个程序封闭地执行初始给定的逻辑，它的“信息秩序”最终会被耗尽，逻辑会退化，错误会堆积。一个真正能够持续进行有效推理的系统，必须有办法获取外部信息，并将这些信息内化，用来重构自己的逻辑结构。 这就像我们人类需要用眼睛、耳朵和双手去感知和改变世界一样。*

*正因为如此，这个项目中的自修改指令才不是一种炫技，而是一种生存的必要。它们是这个系统为自己打开的“感官”和“双手”，让它有能力从外部（无论是过去的执行历史、用户的输入，还是另一个程序的逻辑结构）引入“负熵”，去对抗逻辑的衰减，去不断地完善和进化自己的心智。*

*这也正是它和当前以深度学习为代表的AI有根本区别的地方。深度学习更像是一种内源性的优化——它在固定的结构内，通过误差信号来调整参数，就像一块金属通过内部热传导来达到某个最适温度。而我希望构建的系统，是外源性的：它能够因为一次错误而回溯自己的历史，找到病因，然后生长出新的逻辑来防止同样的错误再次发生。它不是变得更接近某个统计上的最优，而是变得更复杂、更有序、更健壮。*

*如果你也对这个方向感兴趣，欢迎一起探索。我们不是在造一个工具，我们是在试着理解：一个程序，究竟能对自己做什么？*
